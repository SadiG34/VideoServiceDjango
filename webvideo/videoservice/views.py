...
class DetailVideo(HitCountDetailView, View):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.is_authenticated:
            user_is_subscribed = Subscription.objects.filter(subscriber=self.request.user,subscribed_to=video.user).exists()
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        subscribed_to = get_object_or_404(User, pk=video.user.pk)
        ...
        Subscription.objects.filter(subscribed_to=video.user).aggregate(subscribers_count=Count('id'))[
            'subscribers_count']
        if self.request.user.is_authenticated:
...
#                                                                        subscribed_to=subscribed_to)
#             if created:
#                 return HttpResponseRedirect(reverse('video-detail', args=[2]))
#             else:
#                 return HttpResponseRedirect(reverse('video-detail', args=[2]))
#         else:
#             return HttpResponseRedirect(reverse('video-detail', args=[2]))
